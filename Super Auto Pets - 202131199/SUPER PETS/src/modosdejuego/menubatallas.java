...
public class menubatallas {
    ...
    {
        int Ndates [] = Objeto.getNdatosA(codigo);//{getDefensa(), getAtaque(), getVida()};
        String Sdates [] = Objeto.getSdatos(codigo);//{getNombre(),getTipo1(),getTipo2(),getTipo3()};
        num++;
        if (pet)
        {
            ...
            if (oro>=2)
            {
                for (int f=0;f<5;f++)
                {
                    ...
                    {//{getDefensa(), getAtaque(), getVida()}; {getNombre(),getTipo1(),getTipo2(),getTipo3()};
                        int []Ndates = Objeto.getNdatosA(codepet);
                        String []Sdates = Objeto.getSdatos(codepet);
                        matriz[f][0]= codepet;
                        matriz[f][1] = Ndates [2];
                        matriz[f][2] = Ndates [1];
                        matriz[f][3] = 1;
